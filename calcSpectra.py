...
gs.update(left=0.22, right=.94, bottom=.145, top=.99, wspace=0.015, hspace=.015)
fig.patch.set_facecolor('white')
ax = plt.subplot(gs[0])

ax.plot(omega, S_exact,linewidth=3,color='k')
ax.plot(omega, S,'--',linewidth=3,color='C1')

ax.legend(['Exact','Approx'])

ax.set_xlabel('$\omega$ (rad/s)')
ax.set_ylabel('Spectra') # Look up units of spectra
ax.grid()

fig.savefig('maxwellian_10.png',format='png')
