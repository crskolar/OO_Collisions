# This script checks if our spectra converge 
import numpy as np
import math
...
for n in range(0,nmax+1):
    ...
    [U_i, M_i, chi_i] = calcFromSums(sum_U_i[:(n+1)], sum_M_i[:(n+1)], sum_chi_i[:(n+1)], kpar, kperp, nui, wpi)
    ...
    if n != 0:
        ...
        Re_Ui_error[n-1] = calcIterError(np.real(U_i_old), np.real(U_i))
        Im_Ui_error[n-1] = calcIterError(np.imag(U_i_old), np.imag(U_i))
        Re_Mi_error[n-1] = calcIterError(np.real(M_i_old), np.real(M_i))
        Re_chii_error[n-1] = calcIterError(np.real(chi_i_old), np.real(chi_i))
        Im_chii_error[n-1] = calcIterError(np.imag(chi_i_old), np.imag(chi_i))
    
        S_error[n-1] = calcIterError(S_old, S)
    ...
    ax[10].plot(omega, S, linewidth=3, color=colors[n])
    
    ax[1].plot(omega, np.real(U_i), linewidth=3, color=colors[n])
    ax[3].plot(omega, np.imag(U_i), linewidth=3, color=colors[n])
    ax[5].plot(omega, np.real(M_i), linewidth=3, color=colors[n])
    ax[7].plot(omega, np.real(chi_i), linewidth=3, color=colors[n])
    ax[9].plot(omega, np.imag(chi_i), linewidth=3, color=colors[n])
    ax[11].plot(omega, S, linewidth=3, color=colors[n])
