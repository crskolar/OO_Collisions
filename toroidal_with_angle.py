...
fileDir = 'C:/Users/Chirag/Documents/O+O/data/toroidal_ions/'
...
vperp = np.arange(-4*vthiperp,4*vthiperp+dvperp,dvperp)


LOS_temps = np.loadtxt(fileDir + 'LOS_temp.txt')

[VVperp, VVpar] = np.meshgrid(vperp, vpar)
Dstar = 1.8
f = toroidal_norm(VVperp, VVpar, vthiperp, vthipar, Dstar)

# Iterate through the angles
for i in range(8,9):#0,len(angles)):
    
    Tequiv = LOS_temps[i,1]
    print("Angle:", angles[i], "   T_LOS:", Tequiv)
    vthi = (2*kB*Tequiv/mi)**.5
    
    
    # Build the appropriate filename
    fileName = "par2.3_perp2_Goodwin_Te4000_theta" + str(angles[i])
    ...
    im = ax2D.pcolormesh(VVperp, VVpar, f, cmap='inferno', vmin=0, vmax=np.max(f))
